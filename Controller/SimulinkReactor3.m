...
% c =carbon monoxide
% h =hydrogen
function [dx] = SimulinkReactor3(t,x,u) % where t is time (h), x is a vector of state variales, u is a vector of inputs
...
for i=1:n
    if i==1
dWmdt(i) = -(M./(E.*rhog(i))).*(Wm(i)-Wm00)./V + Mrm.*r1(i).*rhoc.*(1-E)./(E.*rhog(i));
dWmedt(i) = -(M./(E.*rhog(i))).*(Wme(i)-Wme00)./V + Mrme.*r2(i).*rhoc.*(1-E)./(E.*rhog(i));
dWwdt(i) = -(M./(E.*rhog(i))).*(Ww(i)-Ww00)./V + Mrw.*r2(i).*rhoc.*(1-E)./(E.*rhog(i));
dWcdt(i) = -(M./(E.*rhog(i))).*(Wc(i)-Wc00)./V + Mrc.*(-r1(i)-r2(i)).*rhoc.*(1-E)./(E.*rhog(i));
dWhdt(i) = -(M./(E.*rhog(i))).*(Wh(i)-Wh00)./V + Mrh.*(-2.*r1(i)-3.*r2(i)).*rhoc.*(1-E)./(E.*rhog(i));
dTdt(i) = (-M.*CpA(i).*(T(i)-T00)./V + (r1(i).*(-Hr1(i)) + r2(i).*(-Hr2(i))).*rhoc.*(1-E))./(E.*rhog(i).*CpA(i) + (1-E).*rhoc.*Cpcat);
    else
